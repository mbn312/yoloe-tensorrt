...
from ._cli_utils import load_yaml_value, parse_imgsz, parse_key_value_args
from .datasets import DatasetValidationError
from .export_cli import (
    ExportKwargsValue,
    add_post_training_export_arguments,
    build_post_training_export_kwargs,
    has_post_training_export_overrides,
)
from .logging_utils import configure_logging
from .training import TrainingError, TrainingResult, UltralyticsOverrideValue, train_model

TrainingKwargsValue: TypeAlias = (
    str | int | float | bool | Path | None | dict[str, UltralyticsOverrideValue] | ExportKwargsValue
)
...
def build_arg_parser() -> argparse.ArgumentParser:
    ...
    add_post_training_export_arguments(export_group)
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.export:
        train_kwargs.update(
            ...
            **build_post_training_export_kwargs(args),
        )
...
def _parse_ultralytics_args(entries: list[str] | None) -> dict[str, UltralyticsOverrideValue]:
    return cast(
        dict[str, UltralyticsOverrideValue],
        parse_key_value_args(entries, option_name="--ultralytics-arg", error_type=_CliArgumentError),
    )
